 ...
 * `-f,--fragment-path`: This argument is required.  Pass the
   ...
   for expression holes can be automatically generated by RexGen (see
   #publications), which we may release in the future.
 * `-t,--test-path`: This argument is required.  Pass the test file
   ...
   the expected model as the argument.  Note that the AUnit tests can
